        ...
        <nav aria-label="Page navigation example">
          ...
          <!-- 이전페이지가 없으면 	버튼 비활성화  -->
            ...
            <!-- 1. page 파라미터 변경함으로써 다음 페이지 조정  -->
              <a class="page-link" href="./cNoticeList?page=${pager.startNum-1}&kind=${pager.kind}" aria-label="Previous">
                <span aria-hidden="true">&laquo;</span>
              </a> 
            </li> 
            <!-- 조정된 다음 페이지를 기준으로 startNum t0 lastNum까지 반복문돌려 블럭 형성  -->
            <c:forEach begin="${pager.startNum}" end="${pager.lastNum}" var="i">
              <li class="page-item"><a class="page-link" href="./cNoticeList?page=${i}&kind=${pager.kind}">${i}</a> </li>
            </c:forEach> 
